...
if __name__=='__main__':
	...
	for p in xrange(len(centers)):
		...
		[maps,dmaps,chisquare]=list(zip(*outputs))
		maps=np.reshape(maps,(nx*ny,3))
		dmaps=np.reshape(dmaps,(nx*ny,3))
		q_map=np.reshape(maps[:,0],(nx,ny))
		u_map=np.reshape(maps[:,1],(nx,ny))
		alpha_map=np.reshape(maps[:,2],(nx,ny))
		dq_map=np.reshape(dmaps[:,0],(nx,ny))
		du_map=np.reshape(dmaps[:,1],(nx,ny))
		dalpha_map=np.reshape(dmaps[:,2],(nx,ny))
		...
		hdulist.writeto('alpha_quiet_mle_cmb'+str(p+1).zfill(1)+'.fits')
