...
if __name__=='__main__':
	...
	simul_cmb=hp.sphtfunc.synfast(cls,1024,fwhm=5.*np.pi/(180.*60.),new=1,pol=1);
	...
	for i in range(num_wl):
		...
		tmp_out=hp.sphtfunc.smoothing(hp.ud_grade(tmp_cmb+np.array([np.zeros(npix1),tmp_q,tmp_u]),nside_out=128,order_in='ring'),fwhm=np.pi/180.,lmax=383,pol=1)
		q_array[i]=tmp_out[1]
		u_array[i]=tmp_out[2]
		sigma_q[i]=hp.sphtfunc.smoothing(hp.ud_grade(tmp_q,nside_out=128,order_in='ring'),fwhm=np.pi/180)
		sigma_u[i]=hp.sphtfunc.smoothing(hp.ud_grade(tmp_u,nside_out=128,order_in='ring'),fwhm=np.pi/180)
