...
def wigner_3j(l1,l2,l3):
	...
	if L % 2 != 0:
		return 0
        a = (-1)**(Decimal(L/2))
        b= Decimal(fac(L-2*l1,exact=1)*fac(L-2*l2,exact=1)*fac(L-2*l3,exact=1))/Decimal(fac(L+1,exact=1))
        c= Decimal(fac(L/2,exact=1))/Decimal(fac(L/2-l1,exact=1)*fac(L/2-l2,exact=1)*fac(L/2-l3,exact=1))
        val = a*np.sqrt(b)*c
        return float(val)
