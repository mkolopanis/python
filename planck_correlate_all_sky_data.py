...
#	iqu_band_j[2]+=sigma_j[1]
	...
	sigma_q_i=hp.ud_grade(sigma_q_i,nside_out=nside_out,order_in=order_i,order_out='ring')
	sigma_u_i=hp.ud_grade(sigma_u_i,nside_out=nside_out,order_in=order_i,order_out='ring')
	sigma_q_j=hp.ud_grade(sigma_q_j,nside_out=nside_out,order_in=order_j,order_out='ring')
	sigma_u_j=hp.ud_grade(sigma_u_j,nside_out=nside_out,order_in=order_j,order_out='ring')
...
def correlate_noise(i_file,j_file,wl_i,wl_j,alpha_file,bands,beam=False,gal_cut=0.):
	print "\t \tComputing Noise Correlation"
...
def plot_mc():
	...
	gal_cut=[-20,-10,-05,00,05,10,20,30]
	bls=hp.gauss_beam(smoothing_scale*np.pi/(180.*60.),383)
	for cut in gal_cut:
		data=np.load('FR_planck_cut_{2:2>02d}.npz'.format(cut))
		...
		cross1_array_in*=1./fsky
		cross2_array_in*=1./fsky
		noise1_array_in*=1./fsky
		noise2_array_in*=1./fsky
		Nau_array_in*=1./fsky
		Naq_array_in*=1./fsky
		Ndu_array_in*=1./fsky
		Ndq_array_in*=1./fsky
		...
		for b in bins:
			...
			delta1=np.sqrt(2.*abs(np.mean(cross1_array_in)**2+np.mean(cross1_arary_in)/2.*(N_dq+N_au)+N_dq*N_au/2.)/((2.*l+1.)*np.sqrt(b**2+dl_eff**2)*fsky))

			N_du=np.mean(Ndu_array_in)
			N_aq=np.mean(Naq_array_in)
			delta2=np.sqrt(2.*abs(np.mean(cross2_array_in)**2+np.mean(cross2_array_in)/2.*(N_dq+N_au)+N_dq*N_au/2.)/((2.*l+1.)*np.sqrt(b**2+dl_eff**2)*fsky))
			...
			if( b != 1):
					...
					tmp_t1=bin_llcl.bin_llcl(ll*theory_in[n]/bls,b)
					tmp_c1=bin_llcl.bin_llcl(ll*cross1_in[n]/bls,b)
		        		tmp_c2=bin_llcl.bin_llcl(ll*cross2_in[n]/bls,b)
		        		
					theory.append(tmp_t1['llcl'])
					                            
					cross1.append(tmp_c1['llcl'])
					cross2.append(tmp_c2['llcl'])
					...
					#noise1.append(tmp_n1['llcl'])
					#tmp_n2=bin_llcl.bin_llcl(ll*noise2_array_in[n]/bls,b)
					#noise2.append(tmp_n2['llcl'])
			
				
			else:
				plot_l=l
				theory=np.multiply(ll/bls,theory_in)
				cross1np.multiply(ll/bls,cross1_array_in)
				#noise1=np.multiply(ll/bls,noise1_array_in)
				cross2=np.multiply(ll/bls,cross2_array_in)
				#noise2=np.multiply(ll/bls,noise2_array_in)
				cosmic1*=ll/bls
				cosmic2*=ll/bls
				delta1*=ll/bls
				delta2*=ll/bls
			...
			#ipdb.set_trace()
			plot_binned.plotBinned((cross)*1e12,dcross*1e12,plot_l,b,'Cross_{0:0>3.0f}x{1:0>3.0f}_FR_cut_{2:0>2d}'.format(bands[i],bands[j],cut), title='Cross {0:0>3.0f}x{1:0>3.0f} FR Correlator'.format(bands[i],bands[j]),theory=theory*1e12,dtheory=cosmic*1e12)
	...
	subprocess.call('mv *01*.png bin_01/'.format(cut), shell=True)
	subprocess.call('mv *05*.png bin_05/'.format(cut), shell=True)
	subprocess.call('mv *10*.png bin_10/'.format(cut), shell=True)
	subprocess.call('mv *20*.png bin_20/'.format(cut), shell=True)
	subprocess.call('mv *25*.png bin_25/'.format(cut), shell=True)
	subprocess.call('mv *50*.png bin_50/'.format(cut), shell=True)
...
def main():
	...
	for cut in gal_cut:
		...
		for i in xrange(len(bands)-1):
			for j in xrange(i+1,len(bands)):
				for n in xrange(N_runs):
					...
					##Do I need this part anymore?
					##Add back in when simulations are complete
					...
					tmp_nosie2_array.append(tmp_n2)
				cross1_array_in.append(np.mean(tmp_cross1_array))
				cross2_array_in.append(np.mean(tmp_cross2_array))
				noise1_array_in.append(np.mean(tmp_noise1_array))
				noise2_array_in.append(np.mean(tmp_noise2_array))
		...
		#read in theory_cls
		theory_in=hp.read_cls('~/Planck/data/faraday/fr_theory_cl.fits')
		...
		np.save('FR_planck_cut_{2:2>02d}.npz'.format(cut),out_dic)
		...
		for b in bins:
			N_dq=np.mean(Ndq_array_in)
			N_au=np.mean(Nau_array_in)
			delta1=np.sqrt(2.*abs((np.mean(cross1_array_in)-np.mean(noise1_array_in))**2+(np.mean(cross1_arary_in)-np.mean(noise1_array_in))/2.*(N_dq+N_au)+N_dq*N_au/2.)/((2.*l+1.)*np.sqrt(b**2+dl_eff**2)*fsky))

			N_du=np.mean(Ndu_array_in)
			N_aq=np.mean(Naq_array_in)
			delta2=np.sqrt(2.*abs((np.mean(cross2_array_in)-np.mean(noise2_array_in))**2+(np.mean(cross2_array_in)+np.mean(noise2_array_in))/2.*(N_dq+N_au)+N_dq*N_au/2.)/((2.*l+1.)*np.sqrt(b**2+dl_eff**2)*fsky))
			...
			#cross_array=np.add(cross1_array,cross2_array)
        		dcross=np.std(cross_array,axis=0,ddof=1)
        		cross=np.average(cross_array,weights=1./dcross**2,axis=0)
        		cosmic=np.sqrt(cosmic1**2+cosmic2**2)
        		delta=np.sqrt(delta1**2+delta2**2)
			#theory1=np.mean(theory1_array,axis=0)
			#dtheory1=np.std(theory1_array,axis=0,ddof=1)
			...
			#dcross1=np.std(np.subtract(cross1_array,noise1),axis=0,ddof=1)
			#ipdb.set_trace()
			plot_binned.plotBinned((cross)*1e12,dcross*1e12,plot_l,b,'Cross_{0:0>3.0f}x{1:0>3.0f}_FR_cut_{2:0>2d}'.format(bands[i],bands[j],cut), title='Cross {0:0>3.0f}x{1:0>3.0f} FR Correlator'.format(bands[i],bands[j]),theory=theory*1e12,dtheory=cosmic*1e12,delta=delta*1e12)
